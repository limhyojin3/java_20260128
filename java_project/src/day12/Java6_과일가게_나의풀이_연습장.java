...
public class Java6_과일가게_나의풀이_연습장 {
...
//				Integer.parseInt(rs.getString("COUNT"));
	...
	public static void searchFruit(Statement stmt) { // DB는 try-catch 안에서 작업 필수!
		...
		try {
			...
			while(rs.next()) { // rs.next() 에 데이터가 있으면 true;
			...
			if(rs.next()) { // where name = name; //과일 이름이 존재할때!
				...
				while(true) {
					...
					if(answer.toUpperCase().equals("Y")) {
						...
						int result = stmt.executeUpdate(sql); //쿼리문 실행후 결과값을 반환
				...
				System.out.println("해당 과일이 존재하지않습니다.");
			}
