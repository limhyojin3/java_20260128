		...
		try {
			...
			while(rs.next()) { //데이터가 있는경우
				...
				if(answer.equals(rs.getString("CAPITAL"))) {
					...
					///ResulSet rs2 = stmt.executeQuery(sql); (X)
					///**(주의)쿼리문 실행할때(executeQuery) => 같은 Statement 사용하면 데이터베이스가 기존연결을 끊고 다시 접속하려고함..

					/// Statement 객체 하나당 ResultSet 객체 하나만 대응가능.
